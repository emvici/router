...
function parsePrerequisites( preq ) {
    ...
    debug( 'process prerequisites', preq );

    var prerequisites = [];
    var keys = Object.keys( Route.PREREQUISITES ),
        handle = function( handlePreq ){
            if( Util.is.Function( handlePreq ) ){

                prerequisites.push( handlePreq );

            } else if( Util.is.String( handlePreq ) && keys.indexOf( handlePreq ) !== -1 ){

                prerequisites.push( Route.PREREQUISITES[handlePreq] );

            } else if( Util.is.Object( handlePreq ) ){

                Util.Object.forEach(handlePreq,function( value, key ){

                    handle( value );

                });

            } else if( Util.is.Array( handlePreq ) ){

                Util.Array.forEach(handlePreq, function( value, key ){
