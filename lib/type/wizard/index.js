...
WizardRoute.construct.defaultOptions = {
     ...
     * validate the model data against the models included in the storeler's uses array.
     *
     * @var boolean
     */
    autoValidate: false,
...
WizardRoute.prototype.dispatch = function () {
    ...
    var currentStep = this.store.currentStep() || this.steps[0];
...
WizardRoute.prototype.generateAvailableSteps = function() {
    var steps = this.steps = Util.Array.filter( this.constructor.steps, function ( step ) {
        return step.branch.id === 'root' || this.store.branched( step.branch.name );
    });
};

WizardRoute.prototype.generateAvailableBranches = function() {
    var branches = this.branches = Util.Array.filter( this.constructor.branches, function ( branch ) {
