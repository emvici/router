    ...
    next = function ( StrOrErr ) {
        ...
        try {
            while ( i < router.Routes.length ) { i++;
                ...
                // Try to match route
                var matchedPath = Route.match( req.url, req.method );

                if( ! matchedPath ) {
                    continue;
                }

                var route = new Route( router, next, matchedPath, req, res );

                debug( "found a route for %s", req.url );

                // If there aren't params, lets get it into the simpliest way!
                if( ! route.params ) {
                    route.dispatch().done(function ( err, str ) { next( err || str || 'route' ); });
                    return;
                }
                ...
                return promise
                    ...
                    .then(function ( str ) { next( str || 'route' ); }, next );
